...
export function UserProfileSettingsForm({
  ...
  const handleAvatarFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      ...
      reader.onloadend = () => {
        setUserAvatarState(reader.result as string);
        toast({ title: "Avatar Updated", description: "Your new avatar is set locally. (Persistence not yet implemented)", className: "glassmorphic font-mono text-xs" });
        // TODO: Persist to Firebase user.photoURL 
        // if (user) { user.updateProfile({ photoURL: reader.result as string }).then(() => toast({...})).catch(err => toast({...})); }
      };
  ...
  const saveName = () => {
    ...
    setCurrentUserNameStateGlobal(finalName);
    setEditingName(false);
    toast({ title: "Display Name Updated", description: `Your name is now "${finalName}". (Persistence not yet implemented)`, className: "glassmorphic font-mono text-xs" });
          ...
          <div className="mt-2 flex items-center gap-2">
            {editingName ? (
              <Input
                ...
                onBlur={() => { if(editingName) saveName();}} // Save if still editing on blur
                autoFocus
