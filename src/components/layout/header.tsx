...
export function Header({ userName, userXP }: HeaderProps) {
  ...
  // Local userXP state removed, using prop
    ...
    <header className="sticky top-0 z-40 w-full glassmorphic rounded-b-md">
      <div className="container mx-auto flex h-14 items-center justify-between px-1 sm:px-1.5 md:px-2">
        <div className="flex items-center min-w-0">
          <SidebarTrigger className="md:hidden mr-0.5 sm:mr-1 text-primary-foreground hover:text-accent h-7 w-7 max-[319px]:h-6 max-[319px]:w-6" />
        ...
        <div className="flex items-center space-x-1 md:space-x-1.5 min-w-0">
          <div className="flex items-center space-x-1 max-[319px]:space-x-0.5 px-1.5 py-1 max-[319px]:px-1 max-[319px]:py-0.5 rounded-md bg-primary/20 backdrop-blur-sm border border-primary/30 min-w-0">
            ...
            <div className="font-mono text-[0.6rem] sm:text-xs font-medium text-primary-foreground flex items-center min-w-0">
              <span className="truncate max-[319px]:max-w-[3ch] max-sm:max-w-[6ch] sm:max-w-[8ch] md:max-w-[12ch]">{userName}</span>
